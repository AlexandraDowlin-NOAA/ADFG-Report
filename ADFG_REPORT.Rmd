...
# **Results:**
Active fishing for the `r maxyr` `r region_abbr` bottom trawl survey occurred between `r dates_conducted` and was the `r survnumber`, standardized multi-species survey in the `r time_series` begun in `r series_begun`. This also comprises the standardized stock assessment data set for the `r region_abbr`. Two charter vessels, the *`r vessel1`* and the *`r vessel2`*, conducted the summer survey and, combined successfully trawled `r s_stations` stations out of `r t_stations` planned stations; `r u_hauls` of the `r a_hauls` trawls attempted were unsuccessful. A total of `r stations_3nm` trawls were attempted within Alaska state waters and collected `r fish_3nm` fish taxa weighing `r scales::comma(fish_3nm_wt)` kg (Table 1). From combined state and offshore waters, the survey collected `r fish_all` fish taxa weighing `r scales::comma(fish_all_wt)` kg in total (Table 2). There were `r inverts_3nm` invertebrate taxa collected from Alaska state waters on the `r maxyr` `r region_abbr` bottom trawl survey with a total weight of `r scales::comma(inverts_3nm_wt)` kg (Table 1). In total, `r inverts_all` invertebrate taxa were collected from combined state and offshore waters and weighed `r scales::comma(inverts_all_wt)` kg (Table 2). The same 4 fish species comprise approximately 80% of the total catch weight from combined state and offshore waters and from within Alaska state waters alone in `r maxyr`. Those species are Pacific ocean perch, Atka mackeral, northern rockfish and walleye pollock. In total, from all state and offshore waters sampled, the survey vouchered `r voucher_all_count` fish and invertebrate taxa and collected `r scales::comma(oto_all)` otoliths pairs from `r oto_taxa` fish species (Table 4). Of the `r voucher_all_count` vouchered taxa, `r fish_taxa_all_count` were fishes and the remaining `r invert_taxa_all_count` were invertebrates. Within Alaska state waters, `r voucher_3nm_count` were vouchered, `r fish_taxa_count_3nm` were fishes and `r invert_taxa_count_3nm` were invertebrates, and `r scales::comma(oto_3nm)` otolith pairs were collected from `r oto_3nm_taxa` fish species (Table 3). Finalized and validated survey data were released on `r data_finalized`. Data products (biomass estimates, length composition, and age composition) were presented to the NPFMC's Joint Plan Team during their annual September meeting. Survey data products are available upon request through the GAP data request repository^[https://github.com/afsc-gap-products/data-requests] and have also been made available over the internet on the Alaska Fisheries Information Network (AKFIN^[https://akfin.psmfc.org/]) and the NMFS Fisheries One Stop Shop (FOSS^[https://www.fisheries.noaa.gov/foss/f?p=215:28]). 
